...
unsigned char * encode_new_order_cross(unsigned char * start,
                                       char side,
                                       unsigned volume,
                                       const std::string & cl_ord_id,
                                       char capacity,
                                       ...
                                       const std::string & algorithmic_indicator)
{
#define FIELD(name) start = encode_field_##name(start, name);
#include "new_order_cross_repeating_groups_fields.inl"
    return start;
}

unsigned char * encode_new_order_cross_parsed(unsigned char * start, const Order & order)
{
    return encode_new_order_cross(start,
                                  convert_side(order.side),
                                  order.volume,
                                  order.cl_ord_id,
                                  convert_capacity(order.capacity),
                                  order.clearing_firm,
                                  std::string(1, convert_account_type(order.account_type)),
                                  order.algorithmic_indicator ? "Y" : "N");
}

unsigned char * encode_new_order_cross_multileg(unsigned char * start,
                                                char side,
                                                unsigned volume,
                                                const std::string & cl_ord_id,
                                                char capacity,
...
{
#define FIELD(name) start = encode_field_##name(start, name);
#include "new_order_cross_multileg_repeating_groups_fields.inl"
    return start;
}

unsigned char * encode_new_order_cross_multileg_parsed(unsigned char * start, const ComplexOrder & order)
{
    return encode_new_order_cross_multileg(start,
                                           convert_side(order.order.side),
                                           order.order.volume,
                                           order.order.cl_ord_id,
                                           convert_capacity(order.order.capacity),
                                           order.order.clearing_firm,
                                           std::string(1, convert_account_type(order.order.account_type)),
                                           convert_legs(order.legs),
                                           order.order.algorithmic_indicator ? "Y" : "N");
}
...
{
    const size_t message_length = new_order_cross_obligatory_count() +
            new_order_cross_repeating_group_count() * (contra_orders.size() + 1) + symbol_field_size;
    std::vector<unsigned char> msg(message_length);
    auto * start = add_request_header(&msg[0], message_length, RequestType::NewCross, seq_no);

    const std::string cross_type = "1";
    const char side = convert_side(agency_order.side);
    const unsigned order_qty = agency_order.volume;
    const char number_of_bitfields = 1;
    const char bitfield = 65;
    const size_t group_count = contra_orders.size() + 1;

#define FIELD(name) start = encode_field_##name(start, name);
#include "new_order_cross_fields.inl"

    start = encode_new_order_cross_parsed(start, agency_order);
    for (size_t i = 0; i < contra_orders.size(); i++) {
        start = encode_new_order_cross_parsed(start, contra_orders[i]);
    }
    encode_text(start, symbol, 8);
...
{
    const size_t message_length = new_order_cross_multileg_obligatory_count() +
            new_order_cross_multileg_repeating_group_count() * (contra_orders.size() + 1) + symbol_field_size;
    std::vector<unsigned char> msg(message_length);
    auto * start = add_request_header(&msg[0], message_length, RequestType::NewCrossMultileg, seq_no);

    const std::string cross_type = "1";
    const char side = convert_side(agency_order.order.side);
    const unsigned order_qty = agency_order.order.volume;
    const char number_of_bitfields = 3;
    const char bitfield1 = 65;
    const char bitfield2 = 0;
    const char bitfield3 = 16;
    const size_t group_count = contra_orders.size() + 1;

#define FIELD(name) start = encode_field_##name(start, name);
#define C_FIELD(name, arg) start = encode_field_##name(start, arg);
#include "new_order_cross_multileg_fields.inl"

    start = encode_new_order_cross_multileg_parsed(start, agency_order);
    for (size_t i = 0; i < contra_orders.size(); i++) {
        start = encode_new_order_cross_multileg_parsed(start, contra_orders[i]);
    }
    encode_text(start, symbol, 8);
